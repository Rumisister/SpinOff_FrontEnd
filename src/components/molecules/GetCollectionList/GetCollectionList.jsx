...
function GetCollectionList({ setNewOpen, isPost = false, postId = '' }) {
  ...
  const requestPreCheckedCollectionList = async () => {
    try {
      const res = await axios({
        method: 'get',
        url: `/api/post/${postId}/collection/all`,
      });
      setCollectionList([
        ...res.data.data.map(r => {
          if (r.collected) dispatch(changeCollection([...collectionIds, r.id]));
          return {
            ...r,
            checked: r.collected,
          };
  ...
  useEffect(() => {
    if (postId) requestPreCheckedCollectionList();
    else requestCollectionList();
  }, []);
  return (
    <Container>
      <SearchContainer>
